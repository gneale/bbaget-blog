...
task :ping do
  begin
    ...
    XMLRPC::Client.new('rpc.pingomatic.com', '/').call('weblogUpdates.extendedPing', 'Grant Neale' , "#{site_url}", "#{site_url}/atom.xml")
  rescue LoadError
...
def create_folder_for_post(file)
  begin
    this_file = file
    folder = Dir::pwd + "/" + 'img' + "/" + 'posts' + "/" + "#{this_file}" + "/"
    # Dir::mkdir(folder)
    puts "the variable file is: #{file}"
  rescue Exception => e
